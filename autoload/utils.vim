...
function! utils#MoveSelection(direction) abort
  ...
  if a:direction ==# 'up' && l:start_line == 1
    " Re-select and return if we're already at the top
    execute "normal! gv"
    return
  endif
  
  if a:direction ==# 'down' && l:end_line == line('$')
    " Re-select and return if we're already at the bottom
    execute "normal! gv"
    return
  endif
  
  " Exit visual mode before moving
  execute "normal! \<Esc>"
  
  if a:direction ==# 'up'
    execute printf("'<,'>move '<-2")
  elseif a:direction ==# 'down'
    execute printf("'<,'>move '>+1")
  endif
  
  " Re-select the moved lines
  execute "normal! gv"
endfunction
